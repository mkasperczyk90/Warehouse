...
public class InventoryRepository(InventoryDbContext context) : IInventoryRepository
{
	...
	public async Task InsertAsync(Domain.Inventory inventory, CancellationToken ct = default) => await _context.AddAsync(inventory, ct);

	public Task UpdateAsync(Domain.Inventory inventory, CancellationToken ct = default)
	{
		_context.Inventory.Update(inventory);
		return Task.CompletedTask;
	}
	public async Task<IReadOnlyList<Domain.Inventory>> ListAsync(CancellationToken ct = default) =>
