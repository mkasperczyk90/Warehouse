...
internal sealed class StockItemRepository(InventoryDbContext context) : IStockItemRepository
{
    ...
    public async Task<IReadOnlyCollection<StockItem>> ListAtAsync(
        LocationCode location, CancellationToken cancellationToken = default) =>
        await context.StockItems.Where(s => s.Location == location).ToListAsync(cancellationToken);
