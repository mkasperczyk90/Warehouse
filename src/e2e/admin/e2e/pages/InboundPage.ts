...
export class InboundPage {
  ...
  // --- Resolve an unknown SKU line (UC-01 exception) -----------------------
  async resolveFirstFlaggedLine() {
    await this.page.getByRole('button', { name: 'Resolve' }).click();
  }
  async fillResolution(sku: string, product: string, alsoCreate: boolean) {
    await this.page.getByLabel('Correct SKU').fill(sku);
    // exact: the "Create this product…" checkbox label also contains "Product".
    await this.page.getByLabel('Product', { exact: true }).fill(product);
    if (alsoCreate) await this.page.getByLabel(/Create this product/).check();
  }
  async submitResolution() {
    await this.page.getByRole('button', { name: 'Map line' }).click();
  }
  async expectTextGone(text: string) {
