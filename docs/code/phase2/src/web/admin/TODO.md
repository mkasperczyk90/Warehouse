...
- [ ] **Multi-warehouse:** the WH-01 selector is inert and WH-02 is a dead node — make the switch real;
      add a user menu / logout.
...
- [ ] Out of scope by design for now: auth/identity UI, analytics/BI dashboards, external self-service
      portals, the terminal's high-contrast theme (desk app is light-only).
...
- [ ] Per-screen identity in `TopBar` is generic (always "K. Manager / KM"). The prototypes vary it by
      actor (coordinator, inspector). Wire it to the (future) auth context rather than hard-coding.
