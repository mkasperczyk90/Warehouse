...
function buildWorklist(): Worklist {
  ...
  const reviewStocktakes = Object.values(stocktakesById).filter((s) => s.summary.state === 'review');
  ...
  return {
    ...
    queues: [
      {
        key: 'qc',
        count: qcBatches.length,
        items: qcBatches.slice(0, 3).map((b) => ({
          id: b.id,
      ...
      },
      {
        key: 'expiring',
        count: 12,
        shownNote: 'top 4 of 12',
        items: expiringItems,
      },
      {
        key: 'inbound',
        count: 2,
        items: asnList.slice(0, 2).map((a) => ({
          id: a.id,
...
export const handlers = [
  http.get('/api/worklist', () => HttpResponse.json(buildWorklist())),
  http.get('/api/movements', () => HttpResponse.json(movements)),
  http.get('/api/locations', () => HttpResponse.json(allLocations())),
  http.get('/api/search', ({ request }) => {
    const q = new URL(request.url).searchParams.get('q') ?? '';
    return HttpResponse.json(q.trim() ? searchAll(q.trim()) : []);
  }),
  http.get('/api/stock/kpis', () => HttpResponse.json(kpis)),
  http.get('/api/stock/rows', () => HttpResponse.json(rows)),
  ...
  http.get('/api/asn', () => HttpResponse.json(asnList)),
  ...
  http.post('/api/asn', async ({ request }) => {
    const body = (await request.json()) as {
      ...
      warehouse: string;
      dockSlot: string;
      lines: { sku: string; product: string; planned: number; unit: string }[];
    };
    ...
    asnList.unshift({
      ...
      status: 'reserved',
      statusLabel: 'Announced',
      meta: `${body.dockSlot} · ${body.lines.length} lines`,
    });
    asnDetails[id] = {
      id,
      supplier: body.supplier,
      warehouse: body.warehouse,
      dockSlot: body.dockSlot,
      createdBy: 'Created by K. Coordinator',
      status: 'reserved',
      statusLabel: 'Announced',
  ...
  http.post('/api/asn/:id/dock', async ({ params, request }) => {
    const { dock, window } = (await request.json()) as { dock: string; window: string };
    const d = asnDetails[params.id as string];
    if (!d) return new HttpResponse(null, { status: 404 });
    d.dockSlot = window ? `${dock} · ${window}` : dock;
    const s = asnList.find((a) => a.id === params.id);
    if (s) s.meta = `${d.dockSlot} · ${d.lines.length} lines`;
    return new HttpResponse(null, { status: 204 });
  }),
  http.get('/api/asn/:id/receiving', ({ params }) => {
    const d = asnDetails[params.id as string];
    if (!d) return new HttpResponse(null, { status: 404 });
    const lines = d.lines.map((l, i) => {
      // Demo progress: first line received, second receiving, rest pending.
      if (i === 0)
        return { id: l.id, sku: l.sku, product: l.product, expected: l.planned, received: l.planned, unit: l.unit, status: 'received', statusLabel: 'Received' };
      if (i === 1)
        return { id: l.id, sku: l.sku, product: l.product, expected: l.planned, received: Math.floor(l.planned / 2), unit: l.unit, status: 'receiving', statusLabel: 'Receiving' };
      return { id: l.id, sku: l.sku, product: l.product, expected: l.planned, received: 0, unit: l.unit, status: 'pending', statusLabel: 'Pending' };
    });
  ...
  http.post('/api/asn/:id/arrive', ({ params }) => {
    const d = asnDetails[params.id as string];
    if (!d) return new HttpResponse(null, { status: 404 });
    d.status = 'transit';
  ...
  http.post('/api/asn/:id/lines/:lineId/resolve', async ({ params, request }) => {
    const { sku, product, create } = (await request.json()) as {
      sku: string;
      product: string;
      create: boolean;
    };
    const d = asnDetails[params.id as string];
    const line = d?.lines.find((l) => l.id === params.lineId);
    if (!line) return new HttpResponse(null, { status: 404 });
    line.sku = sku;
    line.product = product;
    line.flagged = false;
    line.tracking = 'Batch + BBE';
    if (create && !productsBySku[sku]) {
      productsBySku[sku] = {
        sku,
        name: product,
        ean: sku,
  ...
  http.get('/api/orders', () => HttpResponse.json(orderList)),
  ...
  http.get('/api/dispatch/board', () => HttpResponse.json(dispatchBoard)),
  ...
  http.get('/api/stocktake', () =>
    HttpResponse.json(Object.values(stocktakesById).map(stocktakeListItem)),
  ),
  ...
  http.get('/api/qc/batches', () => HttpResponse.json(qcBatches)),
